"""Pilot Info card — compact tactical layout (theme-only colors).

Opened from feed right-click. Renders local Intel History + cached zKill only;
network work stays on the host app's background workers.
"""
...
CHIP = {
    ...
    "system": ("#1a2412", sb_theme.COLORS["gold"]),
    "ship": ("#2a1c10", "#ffb469"),
...
def _chip(parent, text: str, kind: str = "muted"):
    import tkinter as tk

    bg, fg = CHIP.get(kind, CHIP["muted"])
    tk.Label(parent, text=str(text), bg=bg, fg=fg, padx=6, pady=1,
             font=sb_theme.font(8)).pack(side="left", padx=(0, 3), pady=1)


def _section(parent, title: str):
    import tkinter as tk

    f = tk.Frame(parent, bg=sb_theme.COLORS["bg_nav"], padx=8, pady=6)
    f.pack(fill="x", pady=(0, 5))
    tk.Label(
        f, text=title.upper(), bg=f.cget("bg"), fg=sb_theme.COLORS["fg_muted"],
        font=sb_theme.font(8, bold=True),
    ).pack(anchor="w", pady=(0, 3))
    return f


def open_pilot_info_card(app: Any, profile: dict) -> None:
    ...
    def render_header(show_back: bool = False, back_cmd: Callable | None = None):
        ...
        if last:
            ...
            tk.Label(
                ...
                bg=sb_theme.COLORS["bg_panel"], fg="#5ad7ff",
                font=sb_theme.font(9), wraplength=420, justify="left", anchor="w",
    ...
    def render_snapshot():
        ...
        signals = [
            ...
            for k, n in list(signal_counts().items())[:3]
        ]
    ...
    def render_zkill():
        ...
        if status == "syncing":
            tk.Label(
                ...
                bg=box.cget("bg"), fg="#5ad7ff", font=sb_theme.font(9),
            ).pack(anchor="w")
            return
        ...
        ship, _st = normalized_ship_status(latest_row())
        ...
        def event_row(parent, ev, accent):
            ...
            if km_id:
                link = tk.Label(row, text="↗", bg=row.cget("bg"), fg="#5ad7ff", cursor="hand2", font=sb_theme.font(9, bold=True))
    ...
    def render_sightings():
        ...
        box = _section(body, "Recent sightings")
        recent = profile.get("recent_sightings", []) or []
        if not recent:
    ...
    def render_flags():
        ...
        clear_body()
        render_header(show_back=True, back_cmd=render_summary)
        box = _section(body, "Manual flags")
        choices = [
